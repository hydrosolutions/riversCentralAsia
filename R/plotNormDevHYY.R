...
plotNormDevHYY <- function(data2Plot,dataType,stationID){
  ...
  data_norms <- data2Plot %>% dplyr::mutate(norm_data = base::mean(data,na.rm = TRUE)) %>%
  ...
  if (dataType=='Q'){
    ylabText = 'Q [m3/s]'
    titleText_HYY = base::paste0(stationID,': Deviation from Norm (',data_norms$norm_data[1] %>% base::round(1),' m3/s)')
    ...
    titleText_HYY = base::paste0(stationID,': Deviation from Norm (',data_norms$norm_data[1] %>% base::round(1),' deg. C)')
    titleText_cs = base::paste0(stationID,': Cold Season Deviation from Cold Season Norm (',
    ...
    ylabText = 'P [mm]'
    titleText_HYY = base::paste0(stationID,': Deviation from Norm (',data_norms$norm_data[1] %>% base::round(1),' mm)')
    titleText_cs = base::paste0(stationID,': Cold Season Deviation from Cold Season Norm (',
