...
plotNormDevHYY <- function(data2Plot,dataType,stationID){
  ...
  data_norms <- data2Plot %>%
    dplyr::mutate(norm_data = base::mean(.data$data,na.rm = TRUE)) %>%
    dplyr::mutate(norm_data_cs = base::mean(.data$data_cs, na.rm = TRUE)) %>%
    dplyr::mutate(norm_data_ws = base::mean(.data$data_ws, na.rm = TRUE)) %>%
    dplyr::mutate(delta_data = .data$data - .data$norm_data) %>%
    dplyr::mutate(delta_data_cs = .data$data_cs - .data$norm_data_cs) %>%
    dplyr::mutate(delta_data_ws = .data$data_ws - .data$norm_data_ws)
  # generate titles
  if (dataType == 'Q') {
    ylabText = 'Q [m3/s]'
  ...
  completeHYY <- data_norms %>%
    ggplot2::ggplot(ggplot2::aes(x = .data$hyYear, y = .data$delta_data)) +
    ggplot2::geom_bar(ggplot2::aes(fill = .data$delta_data < 0), stat = "identity") +
    ...
    ggplot2::theme_bw()
  csHYY <- data_norms %>%
    ggplot2::ggplot(ggplot2::aes(x = .data$hyYear, y = .data$delta_data_cs)) +
    ggplot2::geom_bar(ggplot2::aes(fill = .data$delta_data_cs < 0), stat = "identity") +
    ggplot2::scale_fill_manual(guide = "none", breaks = c(TRUE, FALSE),
                               values = c("red", "blue")) +
    ggplot2::xlab('Hydrological year') + ggplot2::ylab(ylabText) +
    ...
    ggplot2::theme_bw()
  wsHYY <- data_norms %>%
    ggplot2::ggplot(ggplot2::aes(x = .data$hyYear, y = .data$delta_data_ws)) +
    ggplot2::geom_bar(ggplot2::aes(fill = .data$delta_data_ws < 0), stat = "identity") +
    ggplot2::scale_fill_manual(guide = "none", breaks = c(TRUE, FALSE),
                               values = c("red", "blue")) +
    ggplot2::xlab('Hydrological year') + ggplot2::ylab(ylabText) +
