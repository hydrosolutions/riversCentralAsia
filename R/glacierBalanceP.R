...
#'   input rgi_elbands.
#' @param prcp_a_eb Annual precipitation on glacier elevation band in mm/a.
#'   Same general layout as melt_a_eb. Requires columns \code{Hyear}, \code{ID},
#'   and \code{P}.
...
#' @examples
#'
glacierBalanceP <- function(melt_a_eb, prcp_a_eb, rgi_elbands) {
  ...
  if (!("P" %in% colnames(prcp_a_eb))) {
    cat("Error. Column P not found in input prcp_a_eb.\n")
    return(NULL)
  }
  ...
  prcp_small <- prcp_a_eb |>
    dplyr::select(.data$Hyear, .data$ID, .data$P) |>
    dplyr::filter(.data$ID %in% rgi_small$ID) |>
    tidyr::pivot_wider(names_from = .data$ID, values_from = .data$P)

  prcp_large <- prcp_a_eb |>
    dplyr::select(.data$Hyear, .data$ID, .data$P) |>
    dplyr::filter(.data$ID %in% rgi_large$ID) |>
    tidyr::pivot_wider(names_from = .data$ID, values_from = .data$P)
...
#' @param P_mma matrix with precipitation rates in mm/a (in columns) per
#'   hydrological year (in rows). Also must contain a column named Hyear.
...
stepWiseGlacierBalance <- function(M_mma, P_mma, A_km2) {
  ...
  colnames(P_mma) <- gsub("_\\d", "", colnames(P_mma))
  ...
  if (dim(M_mma_hist_mat)[1] > 1) {
    ...
    Qimb_m3a_hist <- (P_mma_hist_mat-M_mma_hist_mat)*10^(-3)*A_km2_hist*10^6
    ...
    for (time in c((length(years_hist)-1): 1)) {
      ...
      Qimb_m3a_hist[time, ] <- (P_mma_hist_mat[time, ] - M_mma_hist_mat[time, ])*
  ...
  if (dim(M_mma_fut_mat)[1] > 1) {
    ...
    Qimb_m3a_fut <- (P_mma_fut_mat-M_mma_fut_mat)*10^(-3)*A_km2_fut*10^6
...
stepWiseGlacierBalancePerElBand <- function(M_mma, P_mma, A_km2, V_km3) {
  ...
  if (dim(M_mma_hist_mat)[1] > 1) {
    ...
    Q_m3a_hist <- M_mma_hist_mat*10^(-3)*A_km2_hist*10^6
    # Q_m3a_hist <- glacierTotalAblation_HM(M_mma_hist_mat*10^(-3))
    # Qimb_m3a_hist <- glacierImbalAbl(M_mma_hist_mat)
    Qimb_m3a_hist <- (P_mma_hist_mat-M_mma_hist_mat)*10^(-3)*A_km2_hist*10^6
    ...
    for (glacier in glaciers) {
      ...
      M_elBand <- M_mma_hist_mat[, stringr::str_detect(colnames(M_mma_hist_mat),
                                                       glacier)]
      P_elBand <- P_mma_hist_mat[, stringr::str_detect(colnames(P_mma_hist_mat),
                                                       glacier)]
      ...
      Qimb_elBand <- (P_elBand-M_elBand)*10^(-3)*A_elBand*10^6
      ...
      Qimb_glacier <- as.matrix(rowSums(Qimb_elBand))
      for (time in c((length(years_hist)-1): 1)) {
        ...
        Qimb_glacier[time] <- sum(Qimb_elBand[time, ])
  ...
  if (dim(M_mma_fut_mat)[1] > 1) {
    ...
    for (glacier in glaciers) {
      ...
      M_elBand <- M_mma_fut_mat[, stringr::str_detect(colnames(M_mma_fut_mat),
                                                      glacier)]
      P_elBand <- P_mma_fut_mat[, stringr::str_detect(colnames(P_mma_fut_mat),
                                                       glacier)]
