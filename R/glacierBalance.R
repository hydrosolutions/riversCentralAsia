...
#' @param error_stats A data frame or tibble with error statistics for the
#'   functions and variables used to calculate the glacier mass balance over
#'   time. See details for a list of the required statistics.
...
#'   The following error statistics are required as an input. \cr
#'   # Relative error estimates for the initial state in % \cr
#'   error_stats = tibble( \cr
#'     sV = 0.26,  # Glacier volume \cr
#'     sA = 0.13,  # Glacier area \cr
#'     sTh = 0.13,  # Glacier thickness \cr
#'     sMelt = 2,  # Glacier melt \cr
#'     sVolumeRGIF = 0.31,  # Calculating glacier volume from glacier area \cr
#'     sAreaRGIF = 0.53,  # Calculating glacier area from glacier volume \cr
#'     sQglgrowth = 2.16,  # Calculating total glacier discharge from glacier  \cr
#'     #  thinning rates for growing glaciers \cr
#'     sQglmelt = 1.37,  # Calculating total glacier discharge from glacier  \cr
#'     #  thinning rates for melting glaciers \cr
#'     sImbal = 1.27)  # Calculating glacier imbalance ablation from total  \cr
#'     #  glacier discharge \cr
#'
#' @export
...
glacierBalance <- function(melt_a_eb, rgi_elbands, area_threshold = 1,
  ...
  rgi_elbands <- rgi_elbands |>
    ...
    dplyr::mutate(V_km3 = .data$A_km2 * .data$thickness_m*10^(-3),
                  .lb_V_km3 = ifelse(V_km3*(1-error_stats$sV)<0, 0,
                                     V_km3*(1-error_stats$sV)),
                  .ub_V_km3 = V_km3*(1+error_stats$sV))
...
stepWiseGlacierBalance <- function(M_mma, A_km2) {
  ...
  M_mma_hist_mat <- M_mma |>
    dplyr::filter(.data$Hyear <= 2000) |>
    dplyr::select(-.data$Hyear) |>
    as.matrix()
  ...
  if (dim(M_mma_hist_mat)[1] > 1) {
    ...
    for (time in c((length(years_hist)-1): 1)) {
      ...
      Qimb_m3a_hist[time, ] <- glacierImbalAbl(Q_m3a_hist[time, ])
  ...
  if (dim(M_mma_fut_mat)[1] > 1) {
    ...
    # Initialize glacier discharge, area and volume
    A_km2_fut <- matrix(as.numeric(A_km2), nrow = length(years_fut),
                        ncol = length(A_km2), byrow = TRUE)
    colnames(A_km2_fut) <- colnames(A_km2)
    Q_m3a_fut <- M_mma_fut_mat*10^(-3)*A_km2_fut*10^6
    Qimb_m3a_fut <- glacierImbalAbl(Q_m3a_fut)
    ...
    for (time in c(2:length(years_fut))) {
      ...
      Qimb_m3a_fut[time, ] <- glacierImbalAbl(Q_m3a_fut[time, ])
      V_km3_fut[time, ] <- V_km3_fut[time-1,] + Qimb_m3a_fut[time, ]*10^(-9)
    }
  }
...
stepWiseGlacierBalancePerElBand <- function(M_mma, A_km2, V_km3) {
  ...
  glaciers <- unique(gsub("_\\d", "",
                          colnames(M_mma |> dplyr::select(-.data$Hyear))))
  ...
  if (dim(M_mma_hist_mat)[1] > 1) {
    ...
    A_km2_hist <- matrix(as.numeric(A_km2), nrow = length(years_hist),
                         ncol = length(A_km2), byrow = TRUE)
    colnames(A_km2_hist) <- colnames(A_km2)
    V_km3_hist <- matrix(as.numeric(V_km3), nrow = length(years_hist),
    ...
    for (glacier in glaciers) {
      ...
      Qimb_glacier <- glacierImbalAbl(Q_glacier)
      for (time in c((length(years_hist)-1): 1)) {
        Q_elBand[time, ] <- M_elBand[time, ]*10^(-3)*
          A_elBand[time+1, ]*10^6
        Q_glacier[time] <- sum(Q_elBand[time, ])
        Qimb_glacier[time] <- glacierImbalAbl(Q_glacier[time])
  ...
  if (dim(M_mma_fut_mat)[1] > 1) {
    ...
    A_km2_fut <- matrix(as.numeric(A_km2), nrow = length(years_fut),
                        ncol = length(A_km2), byrow = TRUE)
    colnames(A_km2_fut) <- colnames(A_km2)
    V_km3_fut <- matrix(as.numeric(V_km3), nrow = length(years_fut),
                        ncol = length(V_km3), byrow = TRUE)
