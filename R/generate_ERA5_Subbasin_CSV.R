#' Extract subbasin-level and elevation band-specific hourly time series from
#' ERA5 raster bricks
#'
#' Function extracts precipitation (tp) or temperature (t2m) data from raster
#' bricks and prepares a dataframe for later import in RSMinerve (use
#' write.table() function to export as csv). If tp and t2m have to be exported,
...
#'   (generate_ERA5_Subbasin_CSV assumes the following folder structure:
#'   <dir_ERA5_hourly>/<tp or t2m>/<catchmentName>/*.nc)
#' @param catchmentName Name of catchment for which data should be extracted
#'   (depends on proper local file storage)
...
generate_ERA5_Subbasin_CSV <- function(
    dir_ERA5_hourly,catchmentName,dataType,elBands_shp,startY,endY){
  ...
  for (yr in 1:length(fileList$fName)){
    ...
    subbasin_data <- exactextractr::exact_extract(era_data,elBands_shp_latlon) %>% base::lapply(.,colMeans,na.rm=TRUE)
