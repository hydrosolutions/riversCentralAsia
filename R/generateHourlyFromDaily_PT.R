...
#'
#' @param PT_sim Climate scenario dependent daily precipitation and temperature
#'   station values
#' @param era5_data Full ERA5 data frame
#' @param param Stochastic weather simulation parameters
#' @return List with climate scenario dependent hourly P and T data as direct
...
generateHourlyFromDaily_PT <- function(PT_sim,era5_data,param){
  ...
  for (idx in 1:length(PT_sim)){
    ...
    dateVec <- riversCentralAsia::generateSeqDates(PT_sim[[idx]]$year_min_sim,
                                                   PT_sim[[idx]]$year_max_sim,
                                                   "day")
    dateVec <- dateVec$date %>% tibble::as_tibble() %>% dplyr::rename(date=.data$value)
    P_daily <- dateVec %>%
      tibble::add_column((PT_sim[[idx]]$P_gen$prec_gen / 24) %>%
                           tibble::as_tibble(.name_repair = "unique")) # dividing to mm/hour for later gap filling
    P_daily <- P_daily %>%
      dplyr::bind_rows(P_daily[base::rep(base::nrow(P_daily), 1),] %>%
                         dplyr::mutate(date = .data$date + lubridate::days(1))) # Trick: Add duplicated last line with + 1 day
    P_hourly <- P_daily %>% timetk::pad_by_time(.date_var = .data$date,.by = "hour") %>%
    ...
    # A. CLIMATE SCENARIOS
    Tmin <- PT_sim[[idx]]$T_gen$output$Tn_gen %>% tibble::as_tibble(.name_repair = "unique")
    Tmax <- PT_sim[[idx]]$T_gen$output$Tx_gen %>% tibble::as_tibble(.name_repair = "unique")
    Tmean <- (Tmin + Tmax) / 2
    dateVec <- riversCentralAsia::generateSeqDates(PT_sim[[idx]]$year_min_sim,
    ...
    Tmin_hourly <- Tmin_daily %>%
      timetk::pad_by_time(.date_var = .data$date,.by = "hour") %>%
      tidyr::fill(tidyselect::everything(),.direction = 'down') %>%
      dplyr::filter(dplyr::row_number() <= dplyr::n()-1) # remove that added last day row again.
    Tmax_hourly <- Tmax_daily %>% timetk::pad_by_time(.date_var = .data$date,.by = "hour") %>%
      tidyr::fill(tidyselect::everything(),.direction = 'down') %>%
      dplyr::filter(dplyr::row_number() <= dplyr::n()-1) # remove that added last day row again.
    Tmean_hourly <- Tmean_daily %>% timetk::pad_by_time(.date_var = .data$date,.by = "hour") %>%
      tidyr::fill(tidyselect::everything(),.direction = 'down') %>%
      dplyr::filter(dplyr::row_number() <= dplyr::n()-1) # remove that added last day row again.

    Tmin_hourly_mat <- Tmin_hourly %>% dplyr::select(-.data$date) %>% as.matrix()
    Tmax_hourly_mat <- Tmax_hourly %>% dplyr::select(-.data$date) %>% as.matrix()
    Tmean_hourly_mat <- Tmean_hourly %>% dplyr::select(-.data$date) %>% as.matrix()

    # B. ERA5
    diurnalCycle <- computeDiurnalTemperatureCycle(era5_data,param)
    diurnalCycle_era5_mean0 <- diurnalCycle %>% dplyr::select(-.data$hour) %>%
      dplyr::mutate(dplyr::across(.cols=tidyselect::everything(),~ . - mean(.))) %>%
      tibble::add_column(hour=diurnalCycle$hour,.before = 1)
    diurnalCycle_era5_max <- diurnalCycle_era5_mean0 %>% dplyr::select(-.data$hour) %>%
      dplyr::mutate(dplyr::across(.cols=tidyselect::everything(),~ max(.))) %>%
      tibble::add_column(hour=diurnalCycle$hour,.before = 1)
    diurnalCycle_era5_min <- diurnalCycle_era5_mean0 %>% dplyr::select(-.data$hour) %>%
      dplyr::mutate(dplyr::across(.cols=tidyselect::everything(),~ min(.))) %>%
      tibble::add_column(hour=diurnalCycle$hour,.before = 1)

    diurnalCycle_era5_mean0 <- diurnalCycle_era5_mean0 %>% dplyr::select(-.data$hour)
    diurnalCycle_era5_min <- diurnalCycle_era5_min %>% dplyr::select(-.data$hour)
    diurnalCycle_era5_max <- diurnalCycle_era5_max %>% dplyr::select(-.data$hour)

    # Repeat cycles for the total number of days.
    dC_era5_mean0_rep_mat <- do.call("rbind", base::replicate(n=base::nrow(dateVec),
                                       diurnalCycle_era5_mean0, simplify = FALSE)) %>%
      as.matrix()
    ...
    TP_hourly_scenario <- TP_hourly_scenario %>% tibble::as_tibble(.name_repair = "unique")
    ...
    TP_hourly_scenario <- TP_hourly_scenario %>%
      tibble::add_column(P_hourly %>% dplyr::select(-.data$date), .name_repair = "unique")
    # Fix dates to confirm to RS MINERVE
    fixedDates <- posixct2rsminerveChar(TP_hourly_scenario$date) %>%
      dplyr::rename(date=.data$value)
    TP_hourly_scenario <- TP_hourly_scenario %>% dplyr::select(-.data$date) %>%
      tibble::add_column(fixedDates,.before = 1)
