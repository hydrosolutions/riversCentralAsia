...
assess_fc_qual <- function(df, plot){
  ...
  if (per_max == 12){
    ...
    sd_calc <- df |>
      ...
      tidyr::pivot_wider(names_from = per, values_from = obs,
                         names_sort = T) |>
      stats::na.omit() |>
  ...
  if (plot == TRUE){
    p1 <- res |>
      ggplot2::ggplot(ggplot2::aes(x = per, y = fc_qual,
                                   group = per)) +
    ...
    p2 <- numb_good_nperc |>
      ggplot2::ggplot(ggplot2::aes(x = per, y = good_qual_perc)) +
