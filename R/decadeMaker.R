...
decadeMaker <- function(s,e,type){
  ...
  temp <- zoo::zooreg(decade, frequency = 36, start=as.yearmon(s))
  eom <- seq.Date(as.Date(s),by='month',length.out = ydiff * 12) %>%
    zoo:as.yearmon() %>% as.Date(,frac=1) %>% format('%d') %>% as.numeric
  if (strcmp(type,'end')){
    daysV <- cbind(10,20,eom) %>% t %>% as.vector()
  } else if (strcmp(type,'start')){
    daysV <- cbind(1,11,21) %>% t %>% as.vector()
  }
  temp.Date <- as.Date(time(temp)) + daysV - 1
