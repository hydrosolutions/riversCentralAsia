#' Using ERA5 hourly data, the function computes the diurnal temperature cycle
#' for each station.
#'
#' The function takes the tabular ERA5 input data and computes the diurnal cycle
#' for a period of interest between param$year_min and param$yearmax
#'
...
#'   computed.
#' @return Tibble with average diurnal temperature cycles stored in each column
#'   for each station.
...
computeDiurnalTemperatureCycle <- function(era5_data,param){
  ...
  era5_data <- era5_data %>% dplyr::select(-which(era5_data[5,]=='Q'))
  ...
  era5_data_T <- era5_data %>% dplyr::select(which(era5_data[5,]=='T'))
  # Ensure proper tibble column names
  names(era5_data_T) <- era5_data_T[1,] %>% base::as.character()

  # Generate date sequence
  era5_data_dates <-  riversCentralAsia::generateSeqDates(
    param$year_min, param$year_max, "hour")
