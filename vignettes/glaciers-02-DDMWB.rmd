...
ggplot() + 
  geom_line(data = melt_a |> 
              dplyr::filter(RGIId %in% glaciers[1:6]), 
            aes(date, Melt/1000, colour = RGIId)) + 
  geom_line(data = melt_obs_a, aes(start, -dmdtda, colour = RGIId), linetype = 2) + 
...
```{r}
# TODO copy step-wise glacier balance function to package
```
