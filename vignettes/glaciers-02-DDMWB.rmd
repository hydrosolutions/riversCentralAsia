...
$$
\varepsilon_{Q_{\text{gl,growth}}} = \varepsilon_{dhdt} + \varepsilon_{\text{glacierDischarge_HM,growth}} = 0.18\% + 1.42\% = 1.6\%
$$

$$
\varepsilon_{Q_{\text{gl,melt}}} = \varepsilon_{dhdt} + \varepsilon_{\text{glacierDischarge_HM,melt}} = 0.18\% + 0.63\% = 0.81\%
$$

Similarly the error of the imbalance ablation amounts to 1.27% yielding an error of the imbalance ablation of:

$$
\varepsilon_{Q_{\text{imb,melt}}} = \varepsilon_{Q_{\text{gl,melt}}} + \varepsilon_{\text{glacierImbalAbl}} = 0.81\% + 1.27\% = 2.08\%
$$

The errors stated above relate to initial estimates of the glacier areas, volumes, total ablation and imbalance ablation. The uncertainties are propagated through each annual time step of the model.
