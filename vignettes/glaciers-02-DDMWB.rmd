...
Glaciers smaller than 1 km2 are not sub-divided to limit the use of computational resources. The area threshold of 1 km2 is arbitrary but similar area thresholds have been used in the literature (e.g. @miles_health_2021).
...
```{r, include=FALSE}
# clean up
rm(list = ls()[!(ls() %in% c("data_path", "error_stats"))])
```
...
```{r fig-rgielevationbands, message=FALSE, warning=FALSE, fig.cap = "Average elevation of glacier elevation bands.", out.width="90%"}
# Read DEM 
dem <- raster(paste0(data_path, "GIS/16076_DEM.tif"))
...
```{r, echo=FALSE}
load(paste0(data_path, "GLACIERS/TIparameters.RData"))

# Focus on small glaciers assuming that small glaciers in one basin will have 
# similar properties.  
DDF <- DDFfit |> 
  left_join(rgi |> dplyr::select(RGIId, Area, Zmax, Slope, Aspect) |> 
              st_drop_geometry(), 
            by = c("ID" = "RGIId")) |> 
  mutate("Aspect-180" = ifelse(Aspect > 180, Aspect - 180, Aspect + 180)) |> 
  dplyr::filter(Area < 1, 
                MF <= 10)

# We choose glacier slope to moulate MF
model <- lm(MF ~ Slope, data = DDF)

DDFfit <- DDFfit |>
  rename(RGIId = ID) |>
  # Add the layer IDs
  full_join(rgi_elbands |> st_drop_geometry() |> dplyr::select(RGIId, ID),
            by = c("RGIId" = "RGIId")) |>
  # Add the Slope
  left_join(rgi |> dplyr::select(RGIId, Slope) |> 
              st_drop_geometry() |> 
              distinct(RGIId, .keep_all = TRUE), 
            by = "RGIId")
DDFfit <- broom::augment(model, newdata = DDFfit)

# Assign melt factors to glacier HRUs which have not been calibrated 
DDFfit <- DDFfit |> 
  mutate(MF = ifelse(is.na(MF), .fitted, MF),
         Tth = ifelse(is.na(Tth), 0, Tth))
...
DDFfit <- DDFfit |> 
  mutate(layer = as.numeric(gsub("\\_", "", str_extract(ID, "\\_\\d+$"))))
```


Now we re-calculate the glacier melt using the calibrated data set.
```{r, message=FALSE, warning=FALSE, fig.cap="Glacier discharge in the Atbashy basin."}
...
```{r, message=FALSE, warning=FALSE, fig.cap="Observed versus simulated glacier discharge. 11 years of annual data for 131 glaciers in the basin."}
...
```{r, message=FALSE, warning=FALSE, fig.cap="Future specific glacier melt rates expected in the Atbashy basin."}
...
Under development. 
```{r, message=FALSE, warning=FALSE, fig.cap="Expected future development of glacier balance components.", include=FALSE}
fut_glacier_balance <- fut_glacier_balance |> 
  dplyr::filter(!str_detect(Scenario, "IPSL")) |> 
  mutate(GCM = gsub("\\_ssp\\d{3}$", "", Scenario), 
         SSP = str_extract(Scenario, "ssp\\d{3}$")) |> 
  group_by(Hyear, RGIId, SSP, Variable) |> 
  mutate(.lb = min(Value, na.rm = TRUE), 
         .ub = max(Value, na.rm = TRUE), 
         .m = mean(Value, na.rm = TRUE), 
         # deal with missing values
         .lb = ifelse(is.infinite(.lb), NA, .lb), 
         .ub = ifelse(is.infinite(.ub), NA, .ub), 
         .m = ifelse(is.infinite(.m), NA, .m)) |> 
  ungroup()

fut_glacier_balance_basin <- fut_glacier_balance |> 
  dplyr::filter(Hyear > min(Hyear) & Hyear < max(Hyear), 
                Variable %in% c("A_km2", "V_km3", "Q_m3a", "Qimb_m3a")) |> 
  group_by(Hyear, Variable, Scenario) |> 
  summarise(Value = sum(Value, na.rm = TRUE), 
            GCM = first(GCM), 
            SSP = first(SSP)) |> 
  mutate(SSP = factor(SSP, 
                      levels = c("ssp585", "ssp370", "ssp245", "ssp126"))) |> 
  group_by(Hyear, Variable, SSP) |> 
  mutate(.lb = min(Value, na.rm = TRUE), 
         .ub = max(Value, na.rm = TRUE), 
         .m = mean(Value, na.rm = TRUE), 
         # deal with missing values
...
ggplot() + 
  geom_ribbon(data = fut_glacier_balance_basin, 
         aes(Hyear, ymin = .lb, ymax = .ub, fill = SSP), 
              alpha = 0.2, colour = NA) + 
  geom_line(data = fut_glacier_balance_basin, 
         aes(Hyear, .m, colour = SSP)) + 
  facet_wrap("Variable", scales = "free_y") + 
  scale_colour_viridis_d() + 
  scale_fill_viridis_d() + 
  ylab("Value") + 
...
# Peak water
Under development
```{r, message=FALSE, warning=FALSE, fig.cap="Estimated development of glacier discharge.", include=FALSE}
ggplot(fut_glacier_balance_basin |> 
         dplyr::filter(Variable == "Q_m3a")) + 
  geom_ribbon(aes(Hyear, ymin = .lb, ymax = .ub, fill = SSP), 
              alpha = 0.2, colour = NA) +
  #geom_line(aes(Hyear, .m, colour = SSP)) +
  geom_smooth(aes(Hyear, .m, colour = SSP), linetype = 2, size = 0.4,  
...
# Glacier volume loss
Under development. 
```{r, message=FALSE, warning=FALSE, fig.cap="Estimated development of glacier volume.", include=FALSE}
ggplot(fut_glacier_balance_basin |> 
         dplyr::select(-GCM) |> 
         dplyr::filter(Variable == "V_km3") |> 
         dplyr::select(-c(Variable, Scenario, Value)) |> 
         distinct() |> 
         mutate(firstm = ifelse(Hyear == 2013, .m, NA), 
                firstlb = ifelse(Hyear == 2013, .lb, NA), 
                firstub = ifelse(Hyear == 2013, .ub, NA)) |> 
         group_by(SSP) |> 
         fill(firstm, .direction = "down") |>  
         fill(firstlb, .direction = "down") |> 
         fill(firstub, .direction = "down") |> 
         group_by(Hyear, SSP) |> 
         mutate(V = .m/firstm*100, 
                .lv = .lb/firstlb*100, 
                .uv = .ub/firstub*100) |> 
         ungroup() |> 
         mutate(SSP = factor(SSP, 
                             levels = c("ssp126", "ssp245", "ssp370", 
                                        "ssp585")))) + 
  geom_ribbon(aes(Hyear, ymin = .lv, ymax = .uv, fill = SSP), 
              alpha = 0.2) + 
  geom_line(aes(Hyear, V, colour = SSP)) +
  scale_colour_viridis_d() + 
  scale_fill_viridis_d() + 
  ylab("V [%]") + 
  ylim(c(0, 100)) + 
...
fut_melt_d <- fut_melt_d |> 
  # Average per glacier 
  mutate(RGIId = gsub("\\_\\d+$", "", ID)) |> 
  group_by(date, Scenario, RGIId) |> 
  mutate(Melt_mmd = mean(Melt)) |> 
  ungroup() |> 
  dplyr::select(-Melt)

saveRDS(melt_d |> rename(M_mmd = melt), 
        file = paste0(data_path, 
                      "GLACIERS/pre-processed-hist_daily_glacier_melt.rds"))
saveRDS(fut_melt_d, 
        file = paste0(data_path, 
                      "GLACIERS/pre-processed-fut_daily_glacier_melt.rds"))


saveRDS(glacier_balance, 
        file = paste0(data_path, 
                      "GLACIERS/pre-processed-hist_glacier_balance.rds"))
saveRDS(fut_glacier_balance, 
        file = paste0(data_path, 
                      "GLACIERS/pre-processed-fut_glacier_balance.rds"))
```

# Open Global Glacier Model (OGGM)

The [Open Global Glacier Model](https://oggm.org/){target="_blank"} is a community project which provides open-source glacier modelling tools in python and educational resources. The modeling framework includes comprehensive tutorials and data-pre and post-processing tools. It can be used as an alternative to the glacier modelling work flow presented here.
