---
title: "Glaciers 01 - Introduction & Data"
editor: visual
bibliography: [references.bib, HMA_bibliography.bib]
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Glaciers 01 - Introduction & Data}
...
```{r, rgi, message=FALSE, warning=FALSE, fig.cap="DEM & Glaciers (light gray) of the demo basin.", out.width="90%"}
...
The python package [rgitools](https://github.com/GLIMS-RGI/rgitools) provides functions pre-processing RGI glacier outlines, such as automated data quality checks or glacier hypsometry data[^1].
...
#### How to extract glacier thickness data
...
glacier_thickness <- mask(glacier_thickness |> 
                            projectRaster(crs = crs(dem)), rgi)
```

```{r, thickness, message=FALSE, warning=FALSE, fig.cap = "Glacier thickness by Farinotti et al., 2019", out.width="90%"}
tmap_mode("view")
tm_shape(dem, name = "DEM") + 
  tm_raster(n = 6, 
            palette = terrain.colors(6),
            ...
            legend.show = TRUE, 
            title = "Elevation (masl)") + 
  tm_shape(glacier_thickness, name = "Thickness [m]") +
  tm_raster(n = 6, 
            palette = "Blues",
            legend.show = TRUE, 
...
Note: A newer glacier thickness data set has recently become available from @millan_ice_2022 which can be used as an alternative to the data set described above.
...
The package \code{riversCentralAsia} includes an empirical relationship based on a regression between glacier thinning rates and glacier discharge which allows the estimation of glacier discharge.
...
For most hydrological applications, the interest lies on the glacier discharge, i.e. total ablation from glaciers. Despite their large uncertainties, the simulation results by @miles_health_2021 give us an estimate for glacier melt that we can use to calibrate the simpler temperature index models for glacier melt. However, total Ablation is only available for larger glaciers. Contrary to the glacier elevation change rates which are available for all glaciers in the RGI data set. Unfortunately, the relationship between total glacier ablation and glacier elevation change rates is non-linear and not well defined. Nevertheless, it is one more method to estimate glacier contribution to river discharge (apart from long term average glacier mass balance estimates). It is thus possible to estimate total glacier discharge even for glaciers where model results from @miles_health_2021 are not available.

The derivation of the scaling relationship between glacier elevation change and total ablation is given in the [CAHAM book](https://hydrosolutions.github.io/caham_book/glacier_modeling.html#glacier-discharge-thinning-scaling){target="_blank"}. It is implemented in the function `glacierDischarge_HM()`.
...
The glacier thickness data set is validated only at few locations as measurements of glacier thickness are typically not available. @farinotti_consensus_2019 list an uncertainty range for the volume estimate in regions RGI 13 and 14 of 26% each. A more recent glacier thickness data set by @millan_ice_2022 estimates much larger ice reservoirs in the Himalayan region but similar goodness of fit for the glaciers in the Central Asian region as the Farinotti data set.
